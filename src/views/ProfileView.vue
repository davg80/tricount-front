...
const imageRandom = ref(null)
const gender = ref(getLogger.value.gender)
const email = ref(getLogger.value.email)
const firstname = ref(getLogger.value.firstname)
const lastname = ref(getLogger.value.lastname)
console.log(gender)

onMounted(() => {
  axios.get(`https://randomuser.me/api/?gender=${gender.value}`).then((response) => {
    imageRandom.value = response.data.results[0].picture.large
  })
})
</script>
